...
import { CollectionInfoWithSchema, NestedToken, TokenByIdResult } from '@unique-nft/substrate-client/tokens';
...
import { SearchIndexDBService } from '@app/database/search-index.service';
...
type TokenType = Omit<TokenByIdResult, 'nestingChildTokens'> & {
  nestingChildTokens?: NestedToken & { collection: any }[];
};
...
export class SearchIndexService {
  ...
  constructor(
    ...
    private searchIndexDB: SearchIndexDBService,
  ...
  async addSearchIndexIfNotExists(collectionToken: CollectionToken): Promise<SearchIndex[]> {
    const dbIndex = await this.searchIndexDB.findByTokenId(
      String(collectionToken.tokenId),
      String(collectionToken.collectionId),
      collectionToken?.network || this.network,
    );
    if (dbIndex.length) {
      await this.searchIndexDB.deleteByTokenId(String(collectionToken.tokenId), String(collectionToken.collectionId));
    }
  ...
  async prepareSearchIndex(tokenId: number, collectionId: number): Promise<any> {
    ...
    for (const [, object] of Object.entries(tokenData.token?.attributes)) {
      source.add({
  ...
  private dropProperties(token: TokenType): TokenByIdResult {
    ...
    if (token?.nestingChildTokens) {
      token.nestingChildTokens.forEach((item) => {
        this.dropProperties(item as TokenType);
      });
    }
    return token;
  }
  ...
  async updateSearchIndex(): Promise<void> {
    for (const index of await this.repository.query(`select collection_id, token_id from search_index group by collection_id, token_id`)) {
      await this.searchIndexDB.deleteByTokenId(index.token_id, index.collection_id);
